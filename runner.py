...
class Runner():
    ...
    def run(self):
        ...
        arrival = [info["task_arrival"] for info in self.info_buffer]
        dropped = [info["task_dropped"] for info in self.info_buffer]
        pending_avg = [info["pending_len_avg"] for info in self.info_buffer]
        pending_max = [info["pending_len_max"] for info in self.info_buffer]
        unassigned_avg = [info["unassigned_len_avg"] for info in self.info_buffer]
        unassigned_final = [info["unassigned_len_final"] for info in self.info_buffer]
