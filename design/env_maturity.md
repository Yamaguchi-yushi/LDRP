...
2. **APSP 事前計算でテーブル参照化** — **実装済み** ([src/lare/path/lare_path_module.py](../src/lare/path/lare_path_module.py) `compute_apsp`)。本ファイルの文脈 (大規模化のボトルネック) でも再掲。
...
- **checkpoint metadata**: `factor_dim` は既に保存+検証済み。だが**正規化方式を変えても factor_dim は 10 のままなので検証をすり抜ける** ([future_work.md 項目1 の正規化変更](future_work.md#1-lare-path-因子の正規化-3-因子)時に `factor_version` を同梱すべき)。Lifelong + DBCT で複数設定のモデルを扱うので versioning は要る。
