...
> **2026-05-31 アップデート**: §7.3 / §7.4 で指摘していた学習スケジュールの差(update_freq / train_epochs / 評価期間方式) は **解消済み**. LaRePathModule / LaReTaskModule の `end_episode()` に MARL4DRP の `evaluation_period` ステートマシンを移植し、`_update()` は 1-batch + 1-step に変更. 以下の表は履歴として残す.

LDRP の LaRe-Path は MARL4DRP (`~/MARL4DRP/`) の実装を出発点にしているが、いくつかの構造的・hyperparam 的な差がある。
...
### 7.3 学習 hyperparams (※ 2026-05-31 以降は両者が一致)

| パラメータ | LDRP (現在) | MARL4DRP | 備考 |
|---|---|---|---|
| `buffer_capacity` | 1024 | 1024 (単一) / 512×3 (3-way) | LDRP は単一メモリのみ |
| `min_buffer` | 256 | 256 | 一致 |
| `update_freq` (= 評価期間トリガの ep 周期) | **128** | **128** | 一致 (yaml デフォルト) |
| `batch_size` | 256 | 256 | 一致 |
| `train_epochs` (= 連続更新を行うエピソード数) | **50** | **50** (`evaluation_episodes`) | 一致 (dataclass デフォルト) |
| 学習 loop | `update_freq` ep ごとに評価期間 ON → `train_epochs` 個の連続 ep 末尾で **1 update each (fresh batch)** | 同形 ([MARL4DRP drp_env.py:1340-1382](file:///Users/yamaguchiyuushi/MARL4DRP/drp_env/drp_env.py#L1340-L1382)) | 構造一致 |
| 学習 loss | `MSELoss(Σ_t decoder(z_t), env_return_sum)` | 同形 | 一致 |

**変更前 (〜2026-05-30)**: LDRP は `update_freq=3` + `train_epochs=1`(同一 batch を for ループで多重イテレーション)で、MARL4DRP の `128` + `50`(fresh batch × 50 ep)と挙動が大きく違っていた. 2026-05-31 の改修で評価期間ステートマシンを移植し、現在は等価.
...
| pretrained 時の **MSE 学習** | **`frozen=True` で end_episode が早期 return** → MSE 更新を完全スキップ | reset() で `if not use_pretrained_model` チェックして **memory.push をスキップ** する (`drp_env.py:1253`)。train loop は呼ばれるが学習対象データが入らないので実質スキップ |
